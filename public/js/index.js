    ...
    rooms.forEach((room) => {
      if (room.players.length !== 2) {
        html += `<li class="list-group-item d-flex justify-content-between">
                            <p class="p-0 m-0 flex-grow-1 fw-bold">Live room ${room.players[0].username} - ${room.id}</p>
...
socket.on("play", (ennemyPlayer) => {
  if (ennemyPlayer.socketId !== player.socketId && !ennemyPlayer.turn) {
    ...
    if (ennemyPlayer.win) {
      setTurnMessage("alert-info", "alert-danger", `You loss!`);
    ...
    if (calculateEquality()) {
      setTurnMessage("alert-info", "alert-warning", "Tie!");
      return;
    }

    setTurnMessage("alert-info", "alert-success", "Your");
    player.turn = true;
  } else {
    if (player.win) {
      $("#turn-message").addClass("alert-success").html("You won");
      showRestartArea();
      return;
    }
...
function startGame(players) {
  ...
  if (player.host && player.turn) {
    setTurnMessage("alert-info", "alert-success", "Your turn");
  } else {
    setTurnMessage(
      "alert-success",
      "alert-info",
